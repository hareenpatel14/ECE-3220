...
double meancalculator(int data[], int size);
...
int main(int argc , char **argv)
{
    ...
    if (argc == 1)
    {
        printf("Please enter arguements, if not known type; ./a.out -h. Thank you!\n");
        return 0;
    }
    ...
    for (i = 1; i < argc;)
    {
        ...
        {		// command line scale factor input
            ...
            scalefactor = strtod(argv[i + 1], NULL);
            sflag = 1;
            i = i + 2;
        }
        else if (strcmp(argv[i], "-S") == 0)
    ...
    offset_scale(size);
    double mean_val = meancalculator(data,size);
...
void  load_data(int filenum) // opens the file and then loads the data into an array/ other variables
{
    ...
    sprintf(buffer, "A%d.txt",filenum);
    
    FILE *fp;
    fp = fopen(buffer,"r");
    
    fscanf(fp, "%d %d",&size, &limit);
    //printf("this size of the file is %d and the limit is %d\n", size, limit);
    
    for (i=1;i<size+1;i++)
    {
        fscanf(fp,"%d",&data[i]);// scanfing the numbers from file to array
...
double meancalculator(int data[],int size) // finds the mean of the data
{
